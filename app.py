...
# , cmap=sns.cubehelix_palette(8), annot=True, ax=ax
    ...
    fig4 = sns.catplot(data=df, x="Gender", kind="count", height=2)
    st.pyplot(fig4)
    
    fig5 = sns.catplot(data=df, x="BMI Category", kind="count", height=2)
    st.pyplot(fig5)

    fig6 = sns.catplot(data=df, x="Sleep Disorder", kind="count", height=2)
    st.pyplot(fig6)
